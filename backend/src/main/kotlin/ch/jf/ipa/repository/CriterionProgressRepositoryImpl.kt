...
class CriterionProgressRepositoryImpl : CriterionProgressRepository {
    ...
    override suspend fun createOrUpdate(progress: CriterionProgress): CriterionProgress = DatabaseFactory.dbQuery {
        val serializedRequirements = json.encodeToString(listSerializer, progress.checkedRequirements)
        val existing = CriterionProgressTable.select { CriterionProgressTable.id eq progress.id }
            .singleOrNull()
    ...
    override suspend fun getByPersonId(personId: UUID): List<CriterionProgress> = DatabaseFactory.dbQuery {
    ...
    private fun ResultRow.toProgress(): CriterionProgress {
        ...
        val requirements = requirementsRaw.takeIf { it.isNotBlank() }
